...
# Plot 11: Transaction Count by Application Type
def plot_tx_by_app_type(data):
    fig, ax = plt.subplots(figsize=(12, 6))
    for edges, df in data.items():
        if 'application_type' in df.columns:
            app_counts = df['application_type'].value_counts()
            sns.barplot(x=app_counts.index, y=app_counts.values, label=f"{edges} Edges", ax=ax)
    ax.set_title("Transaction Count by Application Type")
    ax.set_xlabel("Application Type")
    ax.set_ylabel("Transaction Count")
    ax.legend()
    ax.grid(True)
    save_plot(fig, "Transaction_By_App_Type")
